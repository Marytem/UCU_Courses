...
def input_from_file():
	...
	data = f.readline()               #цілий кусок дати
	while not data.startswith("   keywords in use:"):
		data = f.readline()           #цілий кусок дати, поки не починаються ківордс
	lst =[]
	while not data.startswith("5: Sub"): # ківордс почалися і до їх кінця - список по рядках(?)
		data = f.readline().strip()  # без пробілів
		lst.append(data) #який апендиться в лст

	keywords = [ w.split("\t") for w in lst] #keywords = розділяє всі рядки(?) по табах - виходять пари(слово-цифра)
	keywords = [(int(w.split()[1][1:-1]),w.split()[0])# розділяє ще по пробілах - (ч-ло, ліст слів?)
				for lst1 in keywords[:-1] for w in lst1 if w]

	while not data.startswith("8: THE"):# кусок дати доки не почнеться ліст ківордс
		data = f.readline()
	
	film_keywords = {}  
	for line in f: #по лінійці того, що лишилося
...
def find_films(film_keywords, keyw1, keyw2):
	...
	Return number of films that using keywords
	"""
	return sum([len(film_keywords[keyw1]),len(film_keywords[keyw1])])
